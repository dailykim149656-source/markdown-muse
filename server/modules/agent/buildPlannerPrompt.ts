...
- You may choose generate_toc, compare_documents, extract_procedure, or suggest_document_updates when the user explicitly asks for those capabilities.
- Keep reason concise and factual.
- Confidence must be between 0 and 1.
- arguments.query should hold the best search query for drive actions when available.
- arguments.prompt should hold the best generation instruction for draft actions when available.
...
- For the other actions, still choose the best semantic action if it truly matches the user request.
